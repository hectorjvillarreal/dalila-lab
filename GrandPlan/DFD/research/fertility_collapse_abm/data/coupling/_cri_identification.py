#!/usr/bin/env python3
"""
Stage 1.5 — Costa Rica identification analysis (Q1-Q4).
Inputs: coupling/CRI_coupling_annual.csv (this work) + national/CRI_tfr_national.csv (Stage 1).
Outputs: console verdicts + coupling/CRI_identification.csv + charts (lead-lag, velocity).
"""
import csv, os
...
rows=list(csv.DictReader(open(os.path.join(HERE,"CRI_coupling_annual.csv"))))
...
for y in years:
    ...
    agg[y]=num/den
# --- load TFR ---
tfr={int(r["year"]):float(r["value"]) for r in csv.DictReader(open(os.path.join(NAT,"CRI_tfr_national.csv")))}
common=[y for y in years if y in tfr]

def yoy(d,ys): return {ys[i]:d[ys[i]]-d[ys[i-1]] for i in range(1,len(ys))}
def pct(d,y0,y1): return 100*(d[y1]-d[y0])/d[y0]

print("=== Costa Rica coupling (20-39 aggregate, %) vs TFR ===")
...
print("\n=== Q2 velocity (YoY change in 20-39 coupling, pts) ===")
acc_year=min(dc,key=lambda y:dc[y])
print("  steepest single-year coupling drop:", acc_year, f"({dc[acc_year]*100:+.2f} pts)")
...
print("\n=== Q3 cascade: steepest-decline year per band (youngest-first?) ===")
for b in BANDS:
    d=yoy(band_share[b],common); ay=min(d,key=lambda y:d[y])
    print(f"  {b}: steepest {ay} ({d[ay]*100:+.2f}); level {band_share[b][2010]*100:.0f}%->{band_share[b][2024]*100:.0f}%")
...
# write summary csv
with open(os.path.join(HERE,"CRI_identification.csv"),"w",newline="") as f:
...
a1.set_title("Costa Rica — coupling (20-39) vs TFR, 2010-2024"); a1.grid(alpha=.25)
...
fig.tight_layout(); fig.savefig(os.path.join(HERE,"CRI_coupling_vs_tfr.png"),dpi=130); plt.close(fig)
print("\nwrote CRI_identification.csv + CRI_coupling_vs_tfr.png")
