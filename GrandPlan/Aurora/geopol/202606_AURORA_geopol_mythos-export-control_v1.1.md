---
doc_id: 202606_AURORA_geopol_mythos-export-control_v1.1
...
version: 1.1
supersedes: 202606_AURORA_geopol_mythos-export-control_v1.0
revision_summary: >
  Maintenance-run trigger checks (2026-07-24) resolved the GPT-5.6 cross-lab
  question: the two gatings were SEPARATE actions under different instruments
  (§3 rewritten). The June 2 executive-order framework surfaced as the
  institutionalizing mechanism. Beijing trigger moved from exploratory to
  formal MOFCOM consultations; Mistral trigger fired; open-weights-letter
  entry filed, resolving the pending pointer. §4 updated, §8 restated.
project: Aurora
layer: geopolitical
...
- **Third door, narrowing (v1.1 update).** Per the Nadella entry's China note,
  ...
  read on whether the tiered regime bites.
...
## 8. Monitoring triggers (restated at v1.1)
...
- **Formal narrowing** of the export-control regime — statutory or
  ...
  instance.
- **Beijing's consultations maturing into rules** — specifically whether the
  floated public-release ban tier is enacted, and whether K3's promised
  2026-07-27 weight release ships unimpeded (near-term read).
