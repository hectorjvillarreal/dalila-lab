#!/usr/bin/env python3
"""
auto-apparatus — obligation scanner for the Aurora corpus.
...
Disciplines (mirroring spcx_monitor):
  - The corpus documents are the single source of truth. This tool holds NO
    state of its own and never writes outside auto-apparatus/.
...
def render(board, n_docs):
    ...
    lines = [
        ...
        "",
        f"*Generated by `auto-apparatus/apparatus.py scan` on {today} over "
    ...
    if not os.path.exists(RUN_LOG):
        with open(RUN_LOG, "w", encoding="utf-8") as f:
            f.write("# auto-apparatus run log\n\n"
