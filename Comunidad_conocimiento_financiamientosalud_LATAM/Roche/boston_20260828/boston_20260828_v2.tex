...
%
% PENDIENTE PARA CLAUDE CODE (ver instrucciones v2):
%   - Argentina en la Figura 2 (CEPALSTAT, mismo indicador y cosecha).
%   - Sustituir el marcador por la imagen real: wpp2024_lac_broad_age_groups.png
%
...
\begin{frame}{América Latina y el Caribe: población por grandes grupos de edad}
  \begin{center}
    % CLAUDE CODE: sustituir por la imagen oficial del portal WPP 2024,
    % agregado "Latin America and the Caribbean". Conservar el pie de
    % atribución incrustado (CC BY 3.0 IGO). NO usar la versión "LLDC".
...
\begin{frame}{La base no da}
  \begin{columns}[T]
    ...
    \begin{column}{0.54\textwidth}
      ...
      % CLAUDE CODE: falta ARGENTINA. Obtener de CEPALSTAT con el MISMO
      % indicador y la MISMA cosecha (ocupados que cotizan o están afiliados
      % a pensiones, 2023-2024). Insertar la coordenada en el orden que le
      % corresponda y añadir la etiqueta al eje simbólico.
      % Si no existe con ese corte exacto: dejar siete barras y activar la
      % nota de la línea marcada abajo. NO sustituir por otro indicador.
      \begin{center}
      \begin{tikzpicture}[scale=0.92, transform shape]
        \begin{axis}[
          ...
          bar width=11pt,
          xmin=0, xmax=92,
          xlabel={\tiny \% de ocupados que cotizan},
          ...
          % CLAUDE CODE: insertar aquí (77,Uruguay) ... con Argentina en su lugar.
        \end{axis}
      \end{tikzpicture}
      \end{center}
      % CLAUDE CODE: activar esta línea SOLO si Argentina no está disponible.
