...
\textit{Preliminary draft -- May 2026}\\
...
This paper's contribution is empirical and structural. Empirically, we calibrate the model to M\'exico and report three quantitative findings. First, under the projected 2050 demographic structure, the equilibrium pension contribution rate roughly doubles under unchanged policy, from 14.5\% to 26.4\%. Second, the welfare incidence of this demographic shock falls asymmetrically by skill, with the sex gap in lifetime welfare closing under aging---a result driven by the sex-asymmetric erosion of the labor-tax wedge that the higher $\tau^p$ implies. Third, and structurally: the 2020 contribution rate combined with the 2020 benefit formula generates a pension financing gap at 2050 demographics that \textit{cannot be financed by a finite stationary stock of government debt}. The menu of feasible long-run policy responses to demographic aging in Latin America is structurally narrower than commonly assumed.
...
We calibrate the model in two steps, following \citet{gourinchas2002consumption}, and organize the parameters into three blocks according to how each is disciplined. The first block is fixed externally from the literature: the curvature of the GHH composite $\gamma$, the inverse Frisch elasticity $\nu_\ell$, the capital share $\alpha$, the depreciation rate $\delta$, the total-factor-productivity level $A$, and the parameters $(\rho, \sigma_\varepsilon)$ of the AR(1) productivity process in equation~\eqref{eq:ar1}. The second block is estimated outside the model in a first step from micro data: sex-specific survival via a Probit, the sex-by-age wage profiles via OLS, and the age-varying health-depreciation schedule $\delta^h_j$ via a two-pass scheme. The third block is calibrated internally by the Simulated Method of Moments (SMM): the preference and health-technology parameters $(\Psi,\, \Xi,\, \xi,\, \overline{H}_0,\, h^{\mathrm{slope}},\, \zeta_h)$, together with the discount factor $\beta$. Conceptually the internal block contains seven parameters; in the current scaffold the active SMM loop estimates six of them, $(\Psi,\, \Xi,\, \xi,\, \overline{H}_0,\, h^{\mathrm{slope}},\, \zeta_h)$, with $\beta$ pinned outside the loop to hit an aggregate capital--output ratio of $K/Y = 3.0$.

The two-step strategy follows \citet{gourinchas2002consumption}. The Block-A and Block-B parameters are identified in partial equilibrium---directly from the data, or from first-step regressions that do not require solving the model---while the Block-C parameters are identified in general equilibrium, since the moments that discipline them (life-cycle medical spending, prime-age hours, and the value of a statistical life) are equilibrium objects that depend on the full vector of household policy functions and on factor prices. Separating the two steps confines the computationally expensive SMM loop to the parameters that genuinely require it.

This draft reports the calibration \emph{protocol} in full and \emph{placeholder numerical values} for the parameters whose identification depends on data inputs still being finalized---the income and expenditure micro-data prepared by Judith M\'endez (ENIGH-derived) and the mortality inputs prepared by Hermilo Cort\'es (CONAPO--UISP life table)---within the remaining calibration window. Final SMM estimates and standard errors will be reported in the post-seminar revision; the qualitative results of the paper---baseline equilibrium magnitudes, the signs of the policy counterfactuals, and the comparative-steady-state findings of Section~\ref{sec:aging}---do not depend on parameter values within the literature-anchored ranges.

\subsection{First-Step Calibration}

The first-step calibration estimates the parameters governing the mortality and labor-productivity processes and the age-dependent health-depreciation schedule.

\subsubsection{Mortality}

We estimate sex-specific survival probabilities using a Probit model. Let $d_{i,t}$ denote an indicator equal to one if individual $i$ is dead at time $t$. Conditional on being alive at time $t$, the probability of dying before period $t+1$ is
\begin{equation}
\label{eq:probit}
\Pr(d_{i,t+1}=1 \mid d_{i,t}=0) \;=\; \Phi\bigl(X_{i,t}'\,\gamma_{\mathrm{m}}\bigr),
\end{equation}
where $\Phi(\cdot)$ is the standard normal cumulative distribution function and $\gamma_{\mathrm{m}}$ is the Probit coefficient vector (subscripted to distinguish from the GHH curvature $\gamma$ in equation~\eqref{eq:utility}). The covariate vector $X_{i,t}$ includes a second-order polynomial in age, a second-order polynomial in health $h_{i,t}$, a sex indicator interacted with the age polynomial, and additional demographic controls (state of residence, marital status). The model's survival probability is recovered as
\begin{equation}
\label{eq:survival_recovery}
\psi^{g}_{j+1}(h') \;=\; 1 - \Phi\bigl(X(j, g, h')'\,\gamma_{\mathrm{m}}\bigr),
\end{equation}
where $X(j, g, h')$ evaluates the covariate vector at the model's age $j$, sex $g$, and next-period health $h'$. The Probit is estimated separately on three datasets, each with sex interactions: the Mexican Health and Aging Study (ENASEM) for M\'exico, the Costa Rican Longevity and Healthy Aging Study (CRELES) for Costa Rica, and the Panamanian \textit{Encuesta de Hogares}.

\subsubsection{Labor Productivity}

We approximate individual labor productivity using observed wages. Taking logs in equation~\eqref{eq:labor_productivity}, the model implies
\begin{equation}
...
\end{equation}
where $\theta_i \in \{\theta_L, \theta_H\}$ is the worker's skill, $h_{i,t}$ is their health stock, and $\eta_{i,t}$ is the AR(1) shock from equation~\eqref{eq:ar1}. Each right-hand-side term maps to a separate set of regressors. We estimate the deterministic block by OLS, regressing log hourly wages on (i) a second-order polynomial in age interacted with sex (recovers $\{e^{g}_{j}\}$), (ii) a skill-group dummy interacted with sex (recovers $\theta_L, \theta_H$), and (iii) the health stock $h$ interacted with the skill dummy (recovers $\varrho(\theta_L), \varrho(\theta_H)$). A quadratic term in $h$ is included as a specification check: under the model it should be zero in expectation; if it is materially different from zero we revisit the functional form.

We then use the residuals from this regression to estimate the stochastic component. We compute the variance--covariance matrix of the residuals across ages and use equally weighted minimum distance to estimate the autoregressive coefficient $\rho$ and the innovation variance $\sigma_{\varepsilon}^{2}$ in equation~\eqref{eq:ar1}.

\subsubsection{Health Depreciation}
\label{subsubsec:health_depreciation}

The age-dependent health-depreciation rate $\delta^h_j$ in equation~\eqref{eq:health_formation} is estimated outside the model from longitudinal observations of self-reported and clinically assessed health stocks. Specifically, for each age cell $j$ we estimate
\begin{equation}
\label{eq:delta_h_estimator}
1 - \delta^h_{j} \;=\; \frac{\mathbb{E}\bigl[\, h_{i,t+1} - H_{j}(m_{i,t}) \mid \text{alive at } t+1,\, j(i,t)=j \,\bigr]}{\mathbb{E}\bigl[\, h_{i,t} \mid j(i,t) = j\,\bigr]},
\end{equation}
i.e., the average period-over-period contraction of the health stock after netting out medical investment. In practice, with $H_j$ not yet estimated at the first stage, we use a two-pass procedure: an initial pass sets $H_j \equiv 0$ to obtain a preliminary $\widehat\delta^h_j$, then $\widehat\delta^h_j$ is updated once after the second-step SMM block has pinned down $(\overline{H}_0, h^{\mathrm{slope}}, \zeta_h)$. Convergence of the two-pass scheme is reached within two iterations in all three country calibrations. Source data come from the Mexican Health and Aging Study (ENASEM), the Costa Rican CRELES, and the Panamanian Health and Aging Study; all are described in Section~\ref{sec:data}.

\subsection{Second-Step Calibration: SMM}
\label{subsec:secondstep}

\subsubsection{Estimated Parameters}
\label{subsec:secondstep_smm}

The parameters not pinned down by external evidence or first-step regressions are estimated jointly by the Simulated Method of Moments. The estimated vector is
\[
\Theta_{\mathrm{SMM}} \;=\; \bigl(\Psi,\; \Xi,\; \xi,\; \overline{H}_0,\; h^{\mathrm{slope}},\; \zeta_h\bigr),
\]
six parameters in total. The three health-technology parameters enter through the production function we adopt following \citet{Carrasco2026},
\begin{equation}
...
\end{equation}
a specific realization of the generic form $H_j(m)=\xi_j\, m^\zeta$ in~\eqref{eq:health_formation} with $\xi_j \equiv \overline{H}_j$ and $\zeta \equiv \zeta_h$. The labor-disutility scale $\Psi$ has no independent economic content and is calibrated as a normalization, so that average hours among employed prime-age men equal one third of the time endowment; the health-amenity parameters $(\Xi,\xi)$ govern the level and curvature of the amenity term $s(h)$ in equation~\eqref{eq:utility}. The discount factor $\beta$ is calibrated \emph{outside} the SMM loop to target an aggregate capital--output ratio of $3.0$; it is the externally anchored seventh parameter of the internal block, which the inner GE solve treats as conditioning information.

\subsubsection{Targeted Moments}

The six SMM parameters are matched to six data moments. With six parameters and six moments the system is exactly identified.
...
$\Xi$ & value of a statistical life $= \$11.6$M (2022 USD) & US-DOT / EPA VSL guidance \\
$\xi$ & income elasticity of medical spending & \citet{hall_jones_2007}; ENASEM \\
$\overline{H}_0$ & mean medical spending, age 25--35 cohort & ENASEM / CRELES / Panam\'a ENH \\
$h^{\mathrm{slope}}$ & log-slope of mean medical spending with age, 25--75 & same as above \\
$\zeta_h$ & cross-sectional income elasticity of $m$ within age bins & same as above \\
\bottomrule
\end{tabular}
\end{center}

\subsubsection{SMM Procedure}

The six parameters are estimated by minimizing the quadratic distance between simulated and empirical moments,
\begin{equation}
...
\end{equation}
with $W$ a diagonal weighting matrix whose entries are the inverse squared standard errors of the data moments.

The objective has an inner--outer structure. At each candidate $\Theta$ the model is solved as a full stationary general equilibrium (Definition~\ref{def:equilibrium}); the inner loop iterates on $(r, w, \tau^p)$ until factor and pension markets clear. The outer optimizer is a multi-start Nelder--Mead simplex (eight starts of at most fifty iterations each, per the scaffold default), with convergence declared when the simplex diameter falls below $10^{-5}$ and the moment-distance gradient---computed by finite differences---has norm below $10^{-4}$.

Simulated moments are computed by drawing $N_{\mathrm{sim}} = 50{,}000$ agents per $(g, \theta)$ from the ergodic distribution of $\eta$ and from the stationary cohort weights $\phi^{g}_j(\theta)$, then propagating each panel forward through the policy functions. Standard errors on $\widehat\Theta_{\mathrm{SMM}}$ are computed by the asymptotic sandwich formula using a numerical Jacobian of $m^{\mathrm{sim}}(\Theta)$ at $\widehat\Theta_{\mathrm{SMM}}$; because $\Xi$ enters $s(h)$ inside the GHH composite, its identification interacts with the externally fixed $\gamma$, which we treat as conditioning information when reporting standard errors.

\subsubsection{Verification Protocol}

The calibration scaffold supports four execution modes---a \textit{smoke} test that solves the partial-equilibrium problem at the starting values, computes the six moments, and exercises the objective; a \textit{parity} test; a \textit{Jacobian} test that adds one full finite-difference Jacobian; and the full \textit{multi-start} optimization. Prior to any calibration run, the parity test verifies that the SMM-scaffold partial-equilibrium solver reproduces the production \texttt{Household-Gender} solver to machine precision, gating on a maximum absolute cohort-mean difference of $10^{-10}$. This ensures that any moment values reported in the post-seminar revision come from a solver algorithm-identical to the general-equilibrium solver used for the policy experiments of Sections~\ref{sec:policy} and~\ref{sec:aging}.

\subsection{Demographic and Type Inputs}
\label{subsec:demographic_inputs}

The model period is five years and the age grid spans $j = 1, \ldots, J = 17$ (ages 20 through 100); retirement is exogenous at $j_R = 10$ (age 65). The gross population growth rate $n_p$ is taken from period-averaged fertility data in Section~\ref{sec:data}. The birth shares $\pi^{g,\theta}_1$ are set to match observed sex-by-education composition at age 20 in the national household surveys, with $\pi^{m,\theta_H}_1 > \pi^{f,\theta_H}_1$ for all three countries. Initial health $h_0$ is normalized to $\bar h$ at birth (full health at age 20). The bounds $\bar a$ and $\bar h$ are set to non-binding values verified ex post on the stationary distribution.

\subsection{Current Calibration Status}
\label{subsec:calib_status}

At the time of writing, the first-step regressions for sex-specific mortality (M\'exico, ENASEM) and the labor-productivity decomposition (M\'exico, ENOE) are in finalization. The second-step SMM block has been verified end-to-end at literature-anchored starting values: the smoke test passes, and parity with the production general-equilibrium solver is verified to $7\times10^{-11}$. The numerical values reported in Section~\ref{sec:aging} reflect the calibration at these starting values, which fall within the standard ranges established in the OLG--health literature. Final SMM estimates, standard errors, and country-specific calibrations for Costa Rica and Panam\'a will be reported in the post-seminar revision, following completion of the data preparation by Judith M\'endez and Hermilo Cort\'es and execution of the multi-start optimization.
...
taxes ($\tau^{\omega}$), consumption taxes embedded in the value-added
...
The model has two skill types $\theta \in \{\theta_L, \theta_H\}$ and within-type heterogeneity in $(a, h, \eta)$. We report welfare at two levels of aggregation. Under the gender extension reported in Section~\ref{sec:aging}, the type index expands from $\theta$ to $(g,\theta)$ and the newborn welfare object becomes $\mathcal{W}_1(g,\theta)$---the direct four-type analogue of the criterion below, evaluated with sex-specific survival $\psi^g_i$.
...
\item \emph{Raising $j_R$} extends the working life, which raises both $L$ and the pension tax base. It is welfare-improving in the model's aggregate utilitarian sense whenever the disutility of an extra working year is smaller than the welfare cost of the equivalent $\kappa$ cut. The incidence is again skewed: low-skill workers have a higher health-productivity elasticity $\varrho(\theta_L)$, so their effective wage falls more sharply with deteriorating health, and an additional working year carries a larger utility cost for them. The reform is therefore distributionally regressive in a non-obvious way that the model captures.
\end{itemize}
...
\begin{enumerate}
\item \emph{Productivity channel:} higher $h$ raises labor productivity $\nu_j(h, \eta;\, g, \theta) = e^g_j \exp\bigl(\theta + \eta + \varrho(\theta)\, h\bigr)$ (see equation~\eqref{eq:labor_productivity}). The strength of the channel is heterogeneous across skill via $\varrho(\theta)$.
...
%% ====================================================================
%% SECTION 7 — COST OF INACTION (COMPARATIVE STEADY STATE, 2050 DEMOGRAPHICS)
%% ====================================================================
\section{Cost of Inaction: Comparative Steady State under 2050 Demographics}
\label{sec:aging}

The policy counterfactuals of Section~\ref{sec:policy} hold demographics fixed at the 2020 calibration and vary policy. The reverse exercise---holding policy at the calibrated 2020 baseline and varying demographics to the 2050 projected structure---gives the \emph{cost of inaction}: the equilibrium the economy converges to when the demographic transition is fully internalized but no policy instrument adjusts. This is a long-run incidence statement, not a transition path; the within-transition incidence on cohorts alive in 2020 is left to future work.

The demographic shift is taken from the UN World Population Prospects (WPP) 2024 revision, medium variant. M\'exico's life expectancy at birth rises from 75.1 years in 2023 to a projected 79.8 by 2050, and the sex gap narrows modestly, from 6.1 to 5.6 years. The total fertility rate falls toward 1.70, implying a contracting entering cohort: the five-year-period population growth rate $n_p$ shifts from $+5.10\%$ (1\% annual) to $-1.98\%$ ($-0.4\%$ annual). The sex-specific survival schedules are derived from the 2020 baseline via a one-parameter Brass-logit shift, calibrated separately for males and females to hit the WPP-projected sex-specific life-expectancy targets; methodology and provenance are documented in Appendix~\ref{app:demog2050}. The resulting demographic old-age dependency ratio under the new structure is $0.74$, exactly $2.0\times$ the baseline value of $0.37$, consistent with ECLAC's projection that old-age dependency at least doubles in M\'exico, Costa Rica, and Panam\'a by 2045 \citep{ECLAC2022}. Within the model's cohort-weighted stationary distribution, the corresponding PAYG dependency ratio $N^R/N^W$ (retirees over workers) rises from $0.29$ to $0.53$.

We solve three stationary equilibria under these 2050 primitives, differing only in fiscal closure: (i) RUN~1---the 2020 baseline under joint closure, in which $\tau^p$ clears the pay-as-you-go (PAYG) pension system and government debt $B$ residually closes the general budget; (ii) C1---2050 demographics under the same joint closure; and (iii) C2---2050 demographics with $\tau^p$ pinned at its 2020 calibrated rate, with pension benefits scaling to maintain PAYG balance ($\text{pen} = \tau^p\, wL / N^R$). The disentanglement of endogenous $\tau^p$ from pinned $\tau^p$ isolates the contribution-rate channel of demographic adjustment from the benefit-side response.

A third closure (C3), in which general taxation absorbs the pension shortfall under a pinned $\tau^p$, is the natural complement to C1 and C2 but is left to the September three-country deliverable. As Section~\ref{subsec:c2_infeasibility} documents, a fourth closure considered---pinning both $\tau^p$ and pension benefits while letting government debt absorb the resulting pension shortfall---fails to admit a finite stationary equilibrium, and serves as the paper's structural motivation for studying the feasible policy menu under demographic pressure.

\subsection{Aggregate Results}
\label{subsec:aging_aggregates}

Table~\ref{tab:aging_comparison} reports the three equilibria. Aging raises the equilibrium pension contribution rate sharply when policy lets $\tau^p$ clear PAYG, from 14.5\% to 26.4\%---a direct consequence of the rise in the dependency ratio $N^R/N^W$ (from 0.29 to 0.53) and the PAYG-balance condition $\tau^p = \kappa\, N^R/N^W$. Capital accumulates substantially under both 2050 closures, with $K$ rising 46\% in C1 and 93\% in C2, reflecting the strengthened life-cycle saving motive at longer life expectancy. The fall in $B/Y$ from 28.3\% to 14.6\% under C1 may appear counterintuitive but reflects the model's general-equilibrium logic: the higher $\tau^p$ absorbs the pension shock, capital accumulation lifts the capital-income tax base, and the residual general government budget loosens. Under C2, where benefits adjust rather than contributions, the precautionary saving response is stronger still and the wage rises by 11\%.

\begin{table}[H]
\centering
\caption{Comparative steady state under 2050 demographics: aggregate equilibria.}
\label{tab:aging_comparison}
\begin{tabular}{lccc}
\toprule
...
$Y$               & 24.506 & 32.990          & 38.942 \\
$r$ (annual)      & 4.81\% & 3.98\%          & 2.96\% \\
$w$               & 0.988  & 1.036           & 1.101  \\
$\tau^p$          & 14.50\% & \textbf{26.38\%} & 14.50\% (pinned) \\
$B/Y$             & 28.30\% & 14.58\%        & 25.32\% \\
$N^R/N^W$         & 0.290  & 0.528           & 0.515  \\
\bottomrule
\end{tabular}

\vspace{0.5em}
\begin{minipage}{0.92\textwidth}
\footnotesize \textit{Notes.} $N^R/N^W$ is the model PAYG dependency ratio (retirees, age 65+, over workers, age 20--64) in the cohort-weighted stationary distribution; it is distinct from the demographic old-age dependency ratio cited in the text. The 2050 demographics are obtained from a Brass-logit shift of the 2020 baseline schedule anchored to UN WPP 2024 medium-variant projections (Appendix~\ref{app:demog2050}). The contribution rate $\tau^p$ is reported at the five-year-period frequency; $r$ is annualized.
\end{minipage}
\end{table}

\subsection{Welfare Incidence by Sex and Skill}
\label{subsec:aging_welfare}

Three substantive findings emerge from the welfare incidence in Table~\ref{tab:welfare_aging} and Figure~\ref{fig:welfare_panel}. First, all four agent types are made worse off by demographic aging under unchanged policy. Second, C2 is welfare-superior to C1 for high-skill types of both sexes---when contributions do not adjust, these households save more for their smaller future pension, and the richer asset stock more than offsets the smaller pension flow---while C2 is welfare-inferior to C1 for low-skill women, who have less ability to self-insure via savings and bear more of the benefit cut than they gain from higher capital income. Third, and central to the gender extension, the sex gap in lifetime welfare \emph{closes} under 2050 demographics: from roughly 5\% (men above women at each skill in RUN~1) to near zero in C1, because the higher $\tau^p$ disproportionately erodes the productive-male labor wedge. The magnitudes of the sex gap depend on the current sex-asymmetry stubs ($e^F = 0.85\,e^M$ and a 25\% female-mortality survival uplift) and will be refined in the September deliverable using sex-specific WPP-2024 calibrated primitives; the \emph{closing direction} of the gap is robust to the calibration.

\begin{table}[H]
\centering
\caption{Lifetime welfare at birth $\mathcal{W}_1(g,\theta)$ by sex and skill.}
\label{tab:welfare_aging}
\begin{tabular}{lcccc}
\toprule
...
\vspace{0.5em}
\begin{minipage}{0.92\textwidth}
\footnotesize \textit{Notes.} Welfare values for RUN~1 and C1 come from a focused-recovery solver that re-solves the household problem at the converged equilibrium prices with $\tau^p$ and $\text{pen}$ consolidated via a three-iteration inner loop; full general-equilibrium replication is reserved for the post-seminar revision (RUN~1 Euler residual $\max\log_{10}=-0.61$; C1 capital-market residual $|A_{\mathrm{dom}}-K-B|/K = 13\%$). C2 welfare comes from a dedicated standalone solver and is bit-precise. The qualitative welfare ordering across types is robust to this precision distinction.
\end{minipage}
\end{table}

\begin{figure}[H]
\centering
% TODO: ensure welfare_panel_gap.png is uploaded to the Overleaf project root.
\includegraphics[width=0.85\textwidth]{welfare_panel_gap.png}
\caption{Lifetime welfare at birth $\mathcal{W}_1(g, \theta)$ across the three reportable equilibria, by sex and skill. The closing sex gap under 2050 demographics, relative to the 2020 baseline, is the qualitative finding of this experiment.}
\label{fig:welfare_panel}
\end{figure}

\subsection{The Feasible Policy Menu: Non-Existence under Debt-Financed Inaction}
\label{subsec:c2_infeasibility}

We now consider an alternative interpretation of the pinned-$\tau^p$ closure that is the conceptual hinge of the paper. Under this interpretation, pension benefits remain at the calibrated PAYG formula ($\text{pen} = \kappa\, wL / N^W$), the contribution rate $\tau^p$ is pinned at the 2020 rate, and the resulting PAYG deficit is absorbed by general government debt. Economically, this is the ``do nothing on the pension side and finance the demographic shock with debt'' scenario.

Under this closure, the model's outer general-equilibrium loop does not converge to a finite stationary equilibrium at M\'exico's 2050 calibration. The mechanism, illustrated in Figure~\ref{fig:c2_crash}, is as follows. The pension deficit pushes the general primary balance deep into deficit; the stationary-debt equation $B = \text{primary} / (r^n - n_p)$ then drives $B$ to large negative values; the implied capital target $K = A_{\mathrm{dom}} - B$ explodes upward; the marginal product of capital falls; the net interest rate $r^n$ crosses the population growth rate $n_p$ from above (visible in the figure at iteration~8); and the stationary-debt equation becomes ill-conditioned as its denominator approaches zero. Beyond that point the iteration oscillates without converging.

This is not a numerical artifact. The model is reporting that, at M\'exico's projected 2050 demographic structure, the 2020 contribution rate combined with the 2020 benefit formula generates a pension financing gap that cannot be financed by a finite stationary stock of government debt: the shortfall pushes the economy into the dynamic-inefficiency region ($r^n < n_p$), where the transversality argument that fixes $B$ at a finite level breaks down. Something must adjust---$\tau^p$ (C1), pension benefits (the C2 column reported above), or general taxation (C3, left to future work). The \emph{menu of feasible long-run policy responses to demographic aging in Latin America is narrower than commonly assumed}: pure debt-financing of the pension shortfall is structurally off the table.

\begin{figure}[H]
\centering
% TODO: ensure c2_interp1_crash_panel.png is uploaded to the Overleaf project root.
\includegraphics[width=\textwidth]{c2_interp1_crash_panel.png}
\caption{Failure to admit a finite stationary equilibrium under the debt-financed-inaction closure (C2, interpretation~1). \textit{Left:} aggregate capital across outer general-equilibrium iterations; capital rises monotonically through iteration~8 and jumps by an order of magnitude (to $K \approx 557$) at iteration~9. \textit{Middle:} the net interest rate $r^n$ and the population growth rate $n_p$ ($-0.40\%$ annual) across iterations; $r^n$ crosses $n_p$ from above at iteration~8 (vertical line), entering the dynamic-inefficiency region (shaded), the point at which the stationary-debt equation becomes ill-conditioned. \textit{Right:} the goods-market residual $|\text{DIFF}/Y|$ across iterations, oscillating without convergence---in contrast to the geometric decay of the RUN~1 baseline.}
\label{fig:c2_crash}
\end{figure}

\subsection{Caveats and Forward Plan}
\label{subsec:aging_caveats}

Four caveats qualify these results, all of which are addressed in the September three-country deliverable. First, the gender decomposition rests on provisional sex-asymmetry stubs ($e^F = 0.85\,e^M$ and a 25\% female-mortality survival uplift) rather than calibrated sex-specific primitives, so the qualitative gender patterns are robust while the magnitudes are stub-dependent. Second, the welfare values for RUN~1 and C1 come from a focused-recovery solver rather than a full general-equilibrium re-solve and are economically sensible but not bit-precise, whereas the C2 welfare is bit-precise from a dedicated standalone driver. Third, the C3 closure (general taxation absorbs the pension shortfall) is not yet computed and so the policy menu reported here is the two-column subset $\{$C1, C2$\}$ rather than the full three-way decomposition. Fourth, the analysis is a comparison of stationary equilibria and therefore silent on within-transition incidence---who bears the cost \emph{while} the demographic transition is occurring---which a transition-path analysis will address.

%% ====================================================================
%% SECTION 8 — FORMALIZATION EXPERIMENT
%% ====================================================================
