...
\hline
Pension contribution $\tau^{p}$ & 13.81\% & \todo{run} & \todo{run} \\
Interest rate $r$ (annual)      & 5.10\%  & \todo{run} & \todo{run} \\
Wage rate $w$                   & 0.972   & \todo{run} & \todo{run} \\
Capital-output $K/Y$ (annual)   & 11.40\% & \todo{run} & \todo{run} \\
Consumption share $C/Y$         & 48.72\% & \todo{run} & \todo{run} \\
Medical share $M/Y$             & 4.82\%  & \todo{run} & \todo{run} \\
Government debt $B/Y$           & 27.50\% & \todo{run} & \todo{run} \\
Pension benefit $\overline{\text{pen}}$ & 1.163 & \todo{run} & \todo{run} \\
Welfare $\mathcal{W}_{1}(\theta_{L})$ & 3.565 & \todo{run} & --- \\
Welfare $\mathcal{W}_{1}(\theta_{H})$ & 4.225 & \todo{run} & --- \\
CEV $\Delta_{\theta_{L}}$ (\%)  & ---     & \todo{run} & --- \\
CEV $\Delta_{\theta_{H}}$ (\%)  & ---     & \todo{run} & --- \\
\hline\hline
\end{tabular}
\begin{minipage}{\linewidth}
\smallskip
\footnotesize
\textit{Notes:} Reform values to be filled in from the converged
$\kappa = 0.30$ stationary equilibrium (\texttt{ge\_summary\_kappa30.csv}).
$\Delta = $ Reform $-$ Baseline. CEV computed against the
$\kappa = 0.50$ baseline using equation~\eqref{eq:cev_newborn}.
\end{minipage}
\end{table}
...
\todo{Run the experiment and fill in the magnitudes.}
...
\hline
Medical share $M/Y$              & 4.82\%  & \todo{run} & \todo{run} \\
Average health $\bar{h}$ at age 60 & \todo{run} & \todo{run} & \todo{run} \\
Pension contribution $\tau^{p}$  & 13.81\% & \todo{run} & \todo{run} \\
Interest rate $r$ (annual)       & 5.10\%  & \todo{run} & \todo{run} \\
Wage rate $w$                    & 0.972   & \todo{run} & \todo{run} \\
Effective labor $L$              & 17.604  & \todo{run} & \todo{run} \\
Output $Y$                       & 26.740  & \todo{run} & \todo{run} \\
Government debt $B/Y$            & 27.50\% & \todo{run} & \todo{run} \\
Welfare $\mathcal{W}_{1}(\theta_{L})$ & 3.565 & \todo{run} & --- \\
Welfare $\mathcal{W}_{1}(\theta_{H})$ & 4.225 & \todo{run} & --- \\
CEV $\Delta_{\theta_{L}}$ (\%)   & ---     & \todo{run} & --- \\
...
\smallskip
\footnotesize
\textit{Notes:} Subsidy values to be filled in from the converged
$\tau^{m} = -0.20$ stationary equilibrium
(\texttt{ge\_summary\_taum20.csv}). $\Delta = $ Subsidy $-$ Baseline.
Average health $\bar{h}$ at age 60 is the mass-weighted mean of
$h_{j=9}$ across skill types. CEV computed against the
$\tau^{m} = 0$ baseline.
\end{minipage}
\end{table}
...
contribution. \todo{Run the experiment and fill in the magnitudes.}
