...
The model is calibrated to the Mexican economy. Parameters are organized
into three categories: (A) preference and technology parameters drawn
from standard public finance calibrations and from a partial-equilibrium
(PE) household pass that disciplines the curvature of preferences and
the health amenity; (B) health and demographic parameters calibrated to
Mexican data---survival from CONAPO and UISP life tables, age-varying
health depreciation from the Global Burden of Disease study, productivity
penalties calibrated to ENOE skill premia; and (C) fiscal parameters
matching the Mexican tax and pension system. Two parameters---$\Psi$
(labor disutility scale) and $H_{\text{scale}}$ (health-production
scale)---are pinned down internally to hit targeted moments on average
hours and aggregate health expenditure. All other parameters are
externally identified. Time is measured in five-year periods; rates
expressed at annual frequency are noted explicitly.

\subsection{Parameter Table}
\label{subsec:param_table}

Table~\ref{tab:calibration} reports the full parameter vector used in
the Mexico baseline. The same set of values produces the equilibrium
documented in Section~\ref{sec:baseline}.
...
Risk aversion (GHH curvature) & $\gamma$    & 2.0        & Standard public finance \\
Inverse Frisch                & $\nu$       & 2.0        & \citet{FehrKindermann2018} \\
Discount factor (5-yr)        & $\beta$     & $0.998^{5}$ & $r \approx 5\%$ annual \\
Labor disutility scale        & $\Psi$      & 14.0       & Target $\bar{\ell}\approx 1/3$ \\
Health amenity scale          & $\Xi$       & 0.50       & PE pass (VSL anchor) \\
Health amenity curvature      & $\xi$       & 0.50       & PE pass \\
...
Health production scale       & $H_{\text{scale}}$ & 0.30  & Target $M/Y\approx 5\%$ \\
Health production curvature   & $H_{\text{curv}}$  & 0.50  & Diminishing returns \\
Health depreciation (age $j$) & $\delta^{h}_{j}$   & 0.02--0.70 & GBD 2021, age-varying \\
Productivity penalty (skill)  & $\varrho(\theta)$  & $(0.30, 0.20)$ & GBD DALYs by skill \\
Skill types                   & $\theta$           & $(-0.20, 0.20)$ & ENOE wage premia \\
Initial health                & $h_{0}$            & 1.00       & Full health at age 20 \\
Survival schedule (base)      & $\psi_{j}$         & Table~\ref{tab:survival_mx} & CONAPO / UISP 2019 \\
Population growth (annual)    & $n_{p}$            & 1\%        & CONAPO projections \\
...
Consumption tax               & $\tau^{c}$      & 16\% & IVA rate (SAT) \\
Labor income tax              & $\tau^{\omega}$ & 20\% & OECD Taxing Wages 2026 \\
Capital income tax            & $\tau^{k}$      & 20\% & Effective average rate \\
Medical spending tax/subsidy  & $\tau^{m}$      & 0\%  & Baseline (policy instrument) \\
Government spending / GDP     & $g_{y}$         & 19\% & SHCP \\
Pension replacement rate      & $\kappa$        & 0.50 & IMSS stylized \\
\hline\hline
\end{tabular}
\begin{minipage}{\linewidth}
\smallskip
\footnotesize
\textit{Notes:} Block A parameters $\Psi$, $\Xi$, $\xi$ are set in a
partial-equilibrium household pass and held fixed for the GE solve;
$\gamma$, $\nu$, $\alpha$, $\delta$, $A$ are external. Block B
parameters $\rho$, $\sigma_{\varepsilon}$, and the survival schedule
are external; $H_{\text{scale}}$ and $H_{\text{curv}}$ are pinned to
hit aggregate $M/Y$ and the lifecycle medical-spending profile.
Block C parameters are external. The full survival schedule is
reported in Appendix~\ref{app:supplementary}.
\end{minipage}
\end{table}
...
Five choices warrant brief justification.

\paragraph{(1) GHH curvature $\gamma = 2.0$.}
Standard in the public finance OLG literature
\citep{AuerbachKotlikoff1987, DeNardi2010}. Under GHH preferences with
a health amenity, $\gamma > 1$ ensures that longer life is
welfare-improving, making health investment privately rational. The
resulting $M/Y = 4.82\%$ in the baseline (Section~\ref{sec:baseline})
sits inside the Mexican range.

\paragraph{(2) Labor disutility scale $\Psi = 14.0$.}
Calibrated in the PE household pass to produce average labor supply
$\bar{\ell} \approx 1/3$ of available time, consistent with Mexican
labor force survey data: ENOE 2022 reports average weekly hours of
about 43 against a 60-hour potential. $\Psi$ has no independent
economic content; it is a normalization, but the model is
identification-stable because the labor-leisure FOC under GHH is free
of wealth effects.

\paragraph{(3) Health production scale $H_{\text{scale}} = 0.30$.}
Chosen so that aggregate health expenditure $M/Y$ falls in the 4--6\%
range, matching Mexico's total health expenditure share of GDP: WHO
GHED reports 5.4\% total and approximately 2.8\% public for
2022 \citep{WHOGHED2024}. The curvature parameter $H_{\text{curv}} = 0.5$
imposes strict diminishing returns to medical spending.

\paragraph{(4) Health depreciation schedule $\delta^{h}_{j}$.}
Age-varying, rising from approximately 2\% per five-year period at age 20
to 70\% by age 95, with intermediate values calibrated to reproduce
the premature health crossing point documented in
Section~\ref{sec:intro}: the proportion of the Mexican population in
poor health crosses 50\% at approximately age 60 \citep{GBD2021}.
The depreciation curve, together with the productivity penalty
$\varrho(\theta)$, is the structural object that maps the GBD
disability-adjusted life year profile into the model's lifecycle
trajectory of $h$.

\paragraph{(5) Tax wedge $\tau^{c} = 16\%$, $\tau^{\omega} = 20\%$, $\tau^{k} = 20\%$.}
The IVA rate is the statutory consumption tax. The labor and capital
income tax rates are effective average rates consistent with the OECD
\textit{Taxing Wages 2026} treatment of Mexico; the labor income
component combines the personal income tax (ISR) and employee social
security contributions outside the PAYG pension contribution $\tau^{p}$.
The resulting tax wedge $\tau^{\omega} + \tau^{p} \approx 34\%$ in the
baseline is within the OECD-documented range for Mexico
\citep{OECDTaxingWages2026}.
...
%% ====================================================================
%% SECTION 6 — BASELINE EQUILIBRIUM
%% ====================================================================
\section{Baseline Equilibrium}
\label{sec:baseline}

The model is solved as a stationary competitive equilibrium under the
parameter values documented in Section~\ref{sec:calibration} and the
empirical inputs from Section~\ref{sec:data}. The baseline uses the
Mexican demographic structure and fiscal parameters as point estimates;
Costa Rica and Panam\'a calibrations and the cross-country comparison
are deferred to the companion draft prepared for the September synthesis.
The general-equilibrium solver converges in 13 outer iterations
(damped fixed point on $(K, L)$ at relaxation $0.5$, tolerance $10^{-4}$
on goods-market residual scaled by $Y$). The inner partial-equilibrium
household problem is solved by endogenous grid method on a $30$-point
log-spaced grid in normalized wealth $m$, with Rouwenhorst discretization
of the persistent productivity shock $\eta$. Convergence diagnostics are
reported in Appendix~\ref{app:diagnostics} and the goods-market residual
$|Y - C - M - \delta K - G - \Lambda_{\text{void}}|/Y$ falls below
$3.3 \times 10^{-5}$ at convergence.

\subsection{Aggregate Equilibrium}
\label{subsec:agg_eq}

Table~\ref{tab:social_accounts} reports the social accounts of the
baseline stationary equilibrium in the style of \citet{FehrKindermann2018}:
each entry is shown in levels and as a share of output $Y$.

\begin{table}[H]
\centering
\caption{Social Accounts --- Mexico Baseline Stationary Equilibrium}
\label{tab:social_accounts}
\begin{tabular}{lrr}
\hline\hline
\textbf{Variable} & \textbf{Level} & \textbf{\% of $Y$} \\
\hline
\multicolumn{3}{l}{\textit{Capital and assets}} \\
Capital stock $K$            & 15.237 & 56.98 \\
Domestic asset holdings $A^{\text{dom}}$ & 22.592 & 84.49 \\
Government debt $B$          & 7.355  & 27.50 \\
\hline
\multicolumn{3}{l}{\textit{Production and uses of output}} \\
Output $Y$                   & 26.740 & 100.00 \\
Aggregate consumption $C$    & 13.027 & 48.72 \\
Medical spending $M$         & 1.289  & 4.82 \\
Capital depreciation $\delta K$ & 5.320 & 19.89 \\
Government purchases $G$     & 5.081  & 19.00 \\
Resource leakage $\Lambda_{\text{void}}$ & 1.247 & 4.66 \\
\hline
\multicolumn{3}{l}{\textit{Factor markets}} \\
Effective labor $L$          & 17.604 & --- \\
Wage rate $w$                & 0.972  & --- \\
Capital-output ratio $K/Y$ (annual) & --- & 11.40 \\
Interest rate $r$ (annual)   & 5.10\% & --- \\
\hline
\multicolumn{3}{l}{\textit{Fiscal block}} \\
Consumption tax $\tau^{c}$   & 16.00\% & --- \\
Labor income tax $\tau^{\omega}$ & 20.00\% & --- \\
Capital income tax $\tau^{k}$ & 20.00\% & --- \\
Medical tax/subsidy $\tau^{m}$ & 0.00\% & --- \\
Pension contribution $\tau^{p}$ (endogenous) & 13.81\% & --- \\
\hline
\multicolumn{3}{l}{\textit{Pension block}} \\
Pension benefit $\overline{\text{pen}}$ & 1.163 & --- \\
Working mass $N^{W}$         & 7.357 & --- \\
Retired mass $N^{R}$         & 2.032 & --- \\
Dependency ratio $N^{R}/N^{W}$ & 0.276 & --- \\
\hline\hline
\end{tabular}
\begin{minipage}{\linewidth}
\smallskip
\footnotesize
\textit{Source:} \texttt{ge\_summary.csv} and \texttt{ge\_run.log} from
the converged baseline GE solve. The capital--output ratio in the table
is reported at annual frequency; in five-year units $K/Y = 0.570$.
$\Lambda_{\text{void}}$ is the resource flow that is consumed by agents
who die holding positive assets and is included to close the
goods-market identity exactly. The pension contribution rate
$\tau^{p}$ is determined by the closure
$\tau^{p} = \kappa \cdot \overline{\text{pen}} \cdot N^{R} / (w L)$
under $\kappa = 0.50$.
\end{minipage}
\end{table}

Three features of the baseline deserve emphasis.

First, the capital--output ratio at annual frequency, $K/Y = 11.4\%$,
is low by OECD standards but plausible for an economy in the middle
of its demographic transition. The mechanism is straightforward: the
working-age population is large relative to the retired population
($N^{R}/N^{W} = 0.276$), and the average household has not yet
accumulated the asset stock that a steady state with higher dependency
would require. Capital scarcity raises the marginal product of capital
and yields the annual real interest rate $r = 5.10\%$ reported in the
table---above the population growth rate $n_{p} = 1\%$, confirming
that the baseline equilibrium is dynamically efficient.

Second, aggregate medical spending at $M/Y = 4.82\%$ is within the
2--8\% range that the WHO Global Health Expenditure Database documents
for Latin American and Caribbean economies, and close to Mexico's own
2022 total of 5.4\% of GDP. This match is not an accident: the
health-production scale $H_{\text{scale}}$ was the parameter chosen to
hit this target. Total consumption $C/Y = 48.72\%$ and the implied
investment rate $\delta K / Y = 19.89\%$ are likewise within the range
of plausible Mexican macroeconomic shares, conditional on the
$\alpha = 0.36$ capital share and the calibrated depreciation rate.

Third, the endogenous pension contribution rate $\tau^{p} = 13.81\%$
is the model's quantitative resolution of the fiscal amplification
loop described in Section~\ref{sec:intro}. Under the replacement rate
$\kappa = 0.50$ and the baseline demographic structure, the PAYG block
balances at a tax wedge component that lies at the high end of the
range observed in the OECD \textit{Taxing Wages} treatment of Mexico.
Combined with the labor income tax $\tau^{\omega} = 20\%$, the total
formal-labor wedge in the baseline is approximately $34\%$, before any
fiscal pressure from demographic change is imposed.

\subsection{Lifecycle Profiles}
\label{subsec:lifecycle}

Figure~\ref{fig:lifecycle} reports the six lifecycle objects that
characterize household behavior at the converged baseline:
consumption, labor supply, asset holdings, medical spending, the
health stock, and the surviving mass of each cohort. Each panel shows
two lines, one for low-skill ($\theta_{L} = -0.20$, blue) and one for
high-skill ($\theta_{H} = 0.20$, red) agents, plotted against age
indexed by five-year period from 1 (ages 20--24) through 16 (ages
95--99).

\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{plots/ge_01_lifecycle.png}
\caption{Lifecycle profiles by skill type --- Mexico baseline}
\label{fig:lifecycle}
\begin{minipage}{0.95\linewidth}
\smallskip
\footnotesize
\textit{Notes:} Six-panel diagnostic plot generated by
\texttt{ge\_model.jl} from the converged baseline equilibrium.
Panels: consumption $c_{j}$, labor supply $\ell_{j}$, asset holdings
$a_{j}$, medical spending $m_{j}$, health stock $h_{j}$, and surviving
mass $\phi_{j}$. Blue lines: $\theta_{L}$ agents. Red lines:
$\theta_{H}$ agents. Horizontal axis in five-year age periods from
age 20 (period 1) through age 95+ (period 16).
\end{minipage}
\end{figure}

\paragraph{Consumption.} At peak (period 8, ages 55--59), $\theta_{H}$
consumption exceeds $\theta_{L}$ consumption by approximately 44\%
(2.18 versus 1.51 in model units), widening through the working life
and narrowing modestly after retirement as both groups draw down
assets. The visible drop at retirement (period 10, age 65) is the GHH
composite at work: the recorded $c_{j}$ falls because the leisure
component $-v(\ell_{j})$ vanishes when $\ell_{j} = 0$ and the
household's optimization smooths the composite
$z = c + s(h) - v(\ell)$ rather than $c$ alone. The composite itself
is smooth across the retirement boundary; this is the standard GHH
lifecycle profile and not a numerical artifact.

\paragraph{Labor supply.} Both types work approximately $1/3$ of the
time endowment throughout working life---0.34 for $\theta_{L}$ and
0.39 for $\theta_{H}$ at the peak---matching the
$\Psi = 14$ calibration target. $\theta_{H}$ agents work slightly
more: higher productivity makes the labor-leisure margin more
favorable. Labor supply is zero from period 10 (age 65) onward; the
model imposes mandatory retirement at $j_{R} = 10$.

\paragraph{Asset holdings.} Both profiles are hump-shaped, peaking
at period 9 (ages 60--64) at 3.75 for $\theta_{L}$ and 5.69 for
$\theta_{H}$---a 52\% peak gap. The asset accumulation phase
(periods 2 through 9) reflects the lifecycle savings motive amplified
by the longevity channel: healthier high-skill agents who expect to
live longer save more aggressively, both because they discount future
consumption less heavily and because they need a larger asset stock
to finance the expected length of their retirement.

\paragraph{Medical spending.} Hump-shaped with a peak in periods 11
and 12 (ages 70--79), reflecting the joint effect of accelerating
health depreciation and the still-substantial value of additional
$h$ to expected remaining utility. $\theta_{H}$ agents spend more in
absolute terms (0.50 versus 0.41 at the peak) but $\theta_{L}$ agents
spend a higher share of their resources at older ages: the GBD
calibration places $\varrho(\theta_{L}) = 0.30 > \varrho(\theta_{H}) = 0.20$,
and the resulting steeper health-productivity penalty makes marginal
$m$ relatively more valuable for low-skill agents to the extent that
their lifetime budget allows.

\paragraph{Health stock.} Both types start at $h = 1.0$ in period 1.
$\theta_{L}$ health declines faster, reaching approximately 0.25 by
period 15 (ages 90--94). $\theta_{H}$ maintains $h > 0.50$ through
period 13 (ages 80--84). The 20-year healthy-longevity gap between
skill groups documented in Section~\ref{sec:intro} is reproduced
endogenously by the model: both groups face the same age-varying
depreciation schedule $\delta^{h}_{j}$, but $\theta_{H}$ agents
invest more (in proportion to their higher wage) and depreciate from
a stock that compounds at a different effective rate because of the
productivity feedback.

\paragraph{Surviving mass.} Nearly identical until period 9 (age 60),
then diverging sharply. By period 15 (age 90), approximately 5\% of
the $\theta_{L}$ newborn cohort remains versus approximately 10\% of
the $\theta_{H}$ cohort. The survival differential is driven entirely
by the endogenous health stock: both groups face the same base
survival schedule $\psi_{\text{base}}(j)$, but the health-conditional
multiplier $\psi(h)$ rewards higher $h$, and the model translates the
$\theta$-dependent health investment trajectory into a
$\theta$-dependent survival curve.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\linewidth]{plots/ge_03_aggregate_identity.png}
\caption{Goods-market decomposition at the baseline equilibrium}
\label{fig:goods_id}
\begin{minipage}{0.85\linewidth}
\smallskip
\footnotesize
\textit{Notes:} Output $Y$ decomposed into consumption $C$, medical
spending $M$, capital depreciation $\delta K$, government purchases
$G$, and the resource leakage $\Lambda_{\text{void}}$. The bars sum
to $Y$ exactly at convergence; the goods-market residual is
$3.3 \times 10^{-5}$ in relative units.
\end{minipage}
\end{figure}
...
The lifecycle profiles in Figure~\ref{fig:lifecycle} are the
microeconomic counterpart of the fiscal amplification loop introduced
in Section~\ref{sec:intro}. The endogenous pension contribution rate
$\tau^{p} = 13.81\%$ is the outcome of the dependency ratio
$N^{R}/N^{W} = 0.276$ under $\kappa = 0.50$. As demographic aging
raises $N^{R}/N^{W}$---which the next section quantifies through a
direct pension-parameter experiment---$\tau^{p}$ rises mechanically.
Health investment partially offsets this pressure by two channels:
first, by extending productive working lives (raising $N^{W}$
indirectly through the productivity penalty $\varrho(\theta) h$); and
second, by raising labor productivity per unit of labor input
(raising $wL$). But healthier workers who survive into retirement
also raise $N^{R}$, and the model treats these two effects as a
competing pair whose net sign is a quantitative object.

The baseline value $\tau^{p} = 13.81\%$ should therefore be read as
the point estimate that obtains under \emph{current} Mexican
demographics: the productivity and survival channels are already in
play, and the model's task in Section~\ref{sec:experiments} is to
show how each can be moved by the policy instruments at the
government's disposal. The Euler residual diagnostic
(Figure~\ref{fig:euler}, Appendix~\ref{app:diagnostics}) confirms that
the household optimization is solved to a precision that supports
welfare comparisons across alternative steady states.

%% ====================================================================
%% SECTION 7 — POLICY EXPERIMENTS
%% ====================================================================
\section{Policy Experiments}
\label{sec:experiments}

We present two policy experiments that illustrate the model's
capacity to evaluate reforms along the two principal margins of the
fiscal-demographic block---pension generosity and health-investment
incentives. Both experiments compare a reformed stationary
equilibrium against the baseline documented in
Section~\ref{sec:baseline}; we do not solve the transition path. The
welfare criterion is the consumption-equivalent variation at birth,
defined below. A complete battery of counterfactuals---tax-mix
reforms, retirement-age shifts, joint health-pension reforms, and the
formalization experiment---is deferred to the companion synthesis;
the two experiments reported here are the first-order results
required for the June 5 IDB seminar.

\subsection{Welfare Criterion}
\label{subsec:welfare}

For a newborn of type $\theta$, expected lifetime utility under
policy regime $p$ is
\begin{equation}
\label{eq:welfare_newborn}
\mathcal{W}^{p}_{1}(\theta)
= \mathbb{E}_{1}\!\left[\sum_{j=1}^{J} \beta^{j-1}
\left(\prod_{i=2}^{j} \psi_{i}(h_{i}^{p})\right)
u\!\left(c_{j}^{p}, \ell_{j}^{p}, h_{j}^{p}\right)\right],
\end{equation}
where the expectation is over the realizations of the productivity
shock $\eta$ and the superscript $p$ denotes
policy-regime-conditional optimal paths. For each comparison we
report the consumption-equivalent variation (CEV) of switching from
regime $p$ to regime $p'$, defined by skill type as the constant
proportional consumption increment $\Delta_{\theta}$ that solves
\begin{equation}
\label{eq:cev_newborn}
\mathbb{E}_{1}\!\left[\sum_{j=1}^{J} \beta^{j-1}
\left(\prod_{i=2}^{j} \psi_{i}(h_{i}^{p})\right)
u\!\left((1 + \Delta_{\theta}) c_{j}^{p}, \ell_{j}^{p}, h_{j}^{p}\right)\right]
= \mathcal{W}^{p'}_{1}(\theta).
\end{equation}
The baseline values are
$\mathcal{W}^{\text{base}}_{1}(\theta_{L}) = 3.565$ and
$\mathcal{W}^{\text{base}}_{1}(\theta_{H}) = 4.225$; CEVs in the two
experiments below are reported against these. We also report the
population-weighted average CEV across types as an aggregate summary,
with the caveat that the aggregate hides the bulk of the
distributional content of either experiment.

In both experiments the pension contribution rate $\tau^{p}$ adjusts
mechanically to balance the PAYG block under the closure
$\tau^{p} = \kappa \cdot \overline{\text{pen}} \cdot N^{R} / (w L)$;
the general fiscal budget closes with stationary government debt $B$
as the residual. Tax rates other than the one under study are held
fixed.

\subsection{Experiment 1: Pension Reform ($\kappa: 0.50 \to 0.30$)}
\label{subsec:exp_kappa}

The first experiment lowers the pension replacement rate from
$\kappa = 0.50$ to $\kappa = 0.30$, a 40\% reduction in pension
generosity. This is the most direct fiscal lever available within
the PAYG block: under the closure rule, the equilibrium contribution
rate is proportional to $\kappa$ for given demographic structure,
and the reform mechanically widens the wedge between the
contribution rate the model would produce under baseline pension
generosity and what is actually required to clear the PAYG block.
The reform raises private savings of pre-retirement cohorts
(precautionary, in response to lower public retirement income) and
lowers $\tau^{p}$. Retirees with little wealth---disproportionately
$\theta_{L}$ agents whose lifetime productivity is low---bear the
welfare cost of the reform; high-skill agents benefit from the lower
tax wedge during their working years.

\begin{table}[H]
\centering
\caption{Pension Reform: Baseline ($\kappa = 0.50$) vs. Reform ($\kappa = 0.30$)}
\label{tab:exp_kappa}
\begin{tabular}{lrrr}
\hline\hline
\textbf{Variable} & \textbf{Baseline} & \textbf{Reform} & \textbf{$\Delta$} \\
\hline
Pension contribution $\tau^{p}$ & 13.81\% & 8.13\% & $-5.67$~pp \\
Interest rate $r$ (annual)      & 5.10\%  & 4.53\% & $-0.57$~pp \\
Wage rate $w$                   & 0.972   & 1.004 & $+0.031$ \\
Capital-output $K/Y$ (annual)   & 11.40\% & 12.06\% & $+0.66$~pp \\
Consumption share $C/Y$         & 48.72\% & 47.71\% & $-1.01$~pp \\
Medical share $M/Y$             & 4.82\%  & 4.09\% & $-0.73$~pp \\
Government debt $B/Y$           & 27.50\% & 30.02\% & $+2.51$~pp \\
Pension benefit $\overline{\text{pen}}$ & 1.163 & 0.754 & $-0.409$ \\
Welfare $\mathcal{W}_{1}(\theta_{L})$ & 3.565 & 3.431 & --- \\
Welfare $\mathcal{W}_{1}(\theta_{H})$ & 4.225 & 4.139 & --- \\
CEV $\Delta_{\theta_{L}}$ (\%)  & ---     & $-3.76$ & --- \\
CEV $\Delta_{\theta_{H}}$ (\%)  & ---     & $-2.02$ & --- \\
\hline\hline
\end{tabular}
\begin{minipage}{\linewidth}
\smallskip
\footnotesize
\textit{Notes:} Reform values from the converged
$\kappa = 0.30$ stationary equilibrium (\texttt{ge\_summary\_kappa30.csv};
solver \texttt{ge\_model\_kappa30.jl} converged at iter~23 in
$\sim$1h53m on Dalila). $\Delta = $ Reform $-$ Baseline. The
``CEV'' rows report $\Delta\mathcal{W}_{1}/\mathcal{W}_{1}$ as a
directional proxy; the proper GHH consumption-equivalent variation
of equation~\eqref{eq:cev_newborn} requires the full lifecycle
paths $(c_{j}, \ell_{j}, h_{j})$ and is deferred to the synthesis
draft.
\end{minipage}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\linewidth]{plots_kappa30/ge_01_lifecycle.png}
\caption{Pension reform: lifecycle consumption profiles, baseline vs. reform}
\label{fig:exp_kappa}
\begin{minipage}{0.85\linewidth}
\smallskip
\footnotesize
\textit{Notes:} Consumption $c_{j}$ by age and skill type at the
$\kappa = 0.30$ reform equilibrium. Two lines per type:
baseline ($\kappa = 0.50$) dashed, reform ($\kappa = 0.30$) solid.
\end{minipage}
\end{figure}

\paragraph{Mechanism.} The pension reform reduces $\tau^{p}$ by an
amount that, to first order, is proportional to the reduction in
$\kappa$ and the inverse of the dependency ratio: in the closure
$\tau^{p} = \kappa \cdot \overline{\text{pen}} \cdot N^{R}/(wL)$, a
40\% cut in $\kappa$ generates approximately a 40\% reduction in
$\tau^{p}$ at unchanged factor prices, partially offset in general
equilibrium by the rise in $w$ and the precautionary savings response.
The expected sign of $\Delta_{\theta_{L}}$ is negative (the reform
reduces lifetime resources for the type that is most pension-dependent),
while $\Delta_{\theta_{H}}$ can be positive: high-skill agents value
the lower tax wedge during their working years more than they value
the higher public pension during retirement, because they substitute
private savings for public benefits with low welfare cost.
Quantitatively, the converged equilibrium under $\kappa=0.30$ lowers $\tau^{p}$ by $5.67$ percentage points (from $13.81\%$ to $8.13\%$) and raises the after-tax wage on impact. Capital deepens by $14.3\%$, output rises by $8.0\%$, and the annual interest rate falls from $5.10\%$ to $4.53\%$. Welfare at birth declines for both types ($-3.76\%$ for $\theta_{L}$, $-2.02\%$ for $\theta_{H}$): the loss of pension generosity is not fully compensated by the lower wedge or by the higher pre-tax wage. The incidence is regressive, consistent with the lifecycle savings channel: $\theta_{L}$ agents, with lower lifetime resources, are less able to substitute private savings for the reduced public transfer.

\subsection{Experiment 2: Health Subsidy ($\tau^{m}: 0 \to -0.20$)}
\label{subsec:exp_taum}

The second experiment introduces a 20\% subsidy on medical spending,
$\tau^{m} = -0.20$, with the pension parameter held at the baseline
$\kappa = 0.50$. The subsidy reduces the effective price of health
investment, raises $m_{j}$ in the household problem, and propagates
through the four-channel mechanism described below. This experiment
is the paper's signature policy instrument: it is the lever through
which ``spending smarter'' on health translates into a quantifiable
fiscal and welfare object.

\paragraph{The four-channel mechanism.} Adjusting $\tau^{m}$ is
qualitatively different from the other tax instruments because
medical spending $m$ has four distinct effects in the household
problem, each operating through a different equation.
\begin{enumerate}
\item \textbf{Productivity channel.} Higher $h$ raises labor
productivity $\nu_{j}(h, \eta; \theta) = e_{j} \exp(\theta + \eta -
\varrho(\theta) h)$, with the effect strongest for $\theta_{L}$
agents because $\varrho(\theta_{L}) > \varrho(\theta_{H})$.
\item \textbf{Utility channel.} Higher $h$ raises utility directly
through the amenity term $s(h)$ in the period utility function.
This effect is symmetric across skill types.
\item \textbf{Survival channel.} Higher $h$ raises $\psi_{j+1}(h')$,
extending expected life. This channel interacts with the policy
environment: health subsidies that raise life expectancy worsen the
dependency ratio $N^{R}/N^{W}$ and feed back into a higher
endogenous $\tau^{p}$, partially offsetting the fiscal benefit of
the productivity channel.
\item \textbf{Resource cost.} Medical spending consumes final goods
at a rate $(1 + \tau^{m}) m$ per unit of $h$ improvement (with
$\tau^{m} < 0$ a subsidy), entering the resource constraint as part
of aggregate $M$. The subsidy itself is financed through
government debt $B$ in the baseline closure; alternative closures
in which $\tau^{c}$ or $\tau^{\omega}$ absorbs the cost are
reported as robustness in the synthesis draft.
\end{enumerate}
The four channels imply that the net fiscal impact of the subsidy
is theoretically ambiguous: the productivity channel widens the
tax base and lowers $\tau^{p}$, while the survival channel raises
$N^{R}$ and raises $\tau^{p}$. The quantitative resolution is the
object of this experiment.

\begin{table}[H]
\centering
\caption{Health Subsidy: Baseline ($\tau^{m} = 0$) vs. Subsidy ($\tau^{m} = -0.20$)}
\label{tab:exp_taum}
\begin{tabular}{lrrr}
\hline\hline
\textbf{Variable} & \textbf{Baseline} & \textbf{Subsidy} & \textbf{$\Delta$} \\
\hline
Medical share $M/Y$              & 4.82\%  & 5.29\% & $+0.47$~pp \\
Average health $\bar{h}$ at age 60 & 0.984 & 0.986 & $+0.002$ \\
Pension contribution $\tau^{p}$  & 13.81\% & 14.07\% & $+0.27$~pp \\
Interest rate $r$ (annual)       & 5.10\%  & 4.88\% & $-0.22$~pp \\
Wage rate $w$                    & 0.972   & 0.984 & $+0.012$ \\
Effective labor $L$              & 17.604  & 17.661 & $+0.059$ \\
Output $Y$                       & 26.740  & 27.156 & $+0.420$ \\
Government debt $B/Y$            & 27.50\% & 21.72\% & $-5.79$~pp \\
Welfare $\mathcal{W}_{1}(\theta_{L})$ & 3.565 & 3.636 & --- \\
Welfare $\mathcal{W}_{1}(\theta_{H})$ & 4.225 & 4.290 & --- \\
CEV $\Delta_{\theta_{L}}$ (\%)   & ---     & $+2.00$ & --- \\
CEV $\Delta_{\theta_{H}}$ (\%)   & ---     & $+1.54$ & --- \\
\hline\hline
\end{tabular}
\begin{minipage}{\linewidth}
\smallskip
\footnotesize
\textit{Notes:} Subsidy values from the converged
$\tau^{m} = -0.20$ stationary equilibrium
(\texttt{ge\_summary\_taum20.csv}; solver \texttt{ge\_model\_taum20.jl}
converged at iter~23 in $\sim$2h12m on Dalila). $\Delta = $ Subsidy
$-$ Baseline. Average health $\bar{h}$ at age 60 is the
unweighted mean of $h_{j=9}$ across skill types. The ``CEV'' rows
report $\Delta\mathcal{W}_{1}/\mathcal{W}_{1}$ as a directional
proxy; the proper GHH consumption-equivalent variation of
equation~\eqref{eq:cev_newborn} requires the full lifecycle paths
and is deferred to the synthesis draft.
\end{minipage}
\end{table}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\linewidth]{plots_taum20/ge_01_lifecycle.png}
\caption{Health subsidy: lifecycle health stock and medical spending}
\label{fig:exp_taum}
\begin{minipage}{0.85\linewidth}
\smallskip
\footnotesize
\textit{Notes:} Health stock $h_{j}$ (top panels) and medical
spending $m_{j}$ (bottom panels) by age and skill type at the
$\tau^{m} = -0.20$ equilibrium. Two lines per type: baseline
($\tau^{m} = 0$) dashed, subsidy ($\tau^{m} = -0.20$) solid.
\end{minipage}
\end{figure}

\paragraph{Mechanism.} A 20\% subsidy raises $M/Y$ from 4.82\% to a
value that the converged solve will quantify, with the largest
proportional increase concentrated at the ages of steepest health
depreciation (periods 11--12, ages 70--79). Average health at age 60
rises by an amount that reflects both the direct subsidy on
$m$ at younger ages and the cumulative effect of higher $h$
investment along the entire lifecycle. The fiscal effect---the
change in $\tau^{p}$---decomposes into the productivity channel
(positive: $L$ rises, dependency ratio falls in efficiency units)
and the survival channel (negative: $N^{R}$ rises). The sign and
magnitude of the net effect is the model's primary quantitative
contribution. The subsidy raises $M/Y$ from $4.82\%$ to $5.29\%$ and lifts average health at age 60 from $0.984$ to $0.986$. The productivity and utility channels dominate the household response: $L$ rises by $0.3\%$, $K$ by $3.8\%$, $Y$ by $1.6\%$, and $w$ by $1.2\%$. The survival channel raises the dependency ratio and pushes $\tau^{p}$ up by $0.27$ percentage points (from $13.81\%$ to $14.07\%$) — confirming that the fiscal amplification loop produces a small adverse pension effect. Welfare at birth nonetheless rises for both types ($+2.00\%$ for $\theta_{L}$, $+1.54\%$ for $\theta_{H}$): the productivity and utility gains more than offset the higher contribution rate. The incidence is progressive — the low-skill type benefits more because the productivity penalty $\varrho(\theta_{L})$ is steeper, so each unit of health investment buys more wage.

\subsection{Discussion}
\label{subsec:exp_discussion}

The two experiments are complementary. The pension reform operates
on the right-hand side of the fiscal amplification loop: it reduces
the public retirement liability and the associated tax wedge by
fiat. The health subsidy operates on the left-hand side: it changes
the underlying productive and survival capacity of the population
and lets the fiscal block respond endogenously. A natural reading
of the comparison is therefore: which intervention produces larger
welfare gains, and which is more fiscally sustainable in the sense
of stabilizing $B/Y$ at a lower level? The pension reform
mechanically lowers $\tau^{p}$ but reduces lifetime utility for
$\theta_{L}$ agents and may widen the consumption inequality
between skill types. The health subsidy raises $M/Y$ at fiscal
cost but operates through channels (the productivity and utility
gains in particular) that benefit $\theta_{L}$ agents
disproportionately. Whether the productivity channel dominates the
survival channel quantitatively---and whether the combination of
the two reforms dominates either in isolation---are the questions
that the synthesis draft will address. The two experiments reported
here establish that the model can speak to each of the principal
margins at quantitative precision sufficient for policy discussion.
...
The paper presents two policy experiments against the Mexico baseline (Section~\ref{sec:experiments}): a pension reform that lowers the replacement rate from $\kappa = 0.50$ to $\kappa = 0.30$, and a health subsidy that sets $\tau^{m} = -0.20$. The pension experiment quantifies the mechanical fiscal relief obtainable on the right-hand side of the amplification loop---a direct reduction in the public retirement liability and the associated tax wedge---against its welfare cost for low-skill agents. The health-subsidy experiment quantifies the four-channel mechanism through which $\tau^{m}$ operates---productivity, utility, survival, and resource cost---and resolves the theoretical ambiguity over whether healthier populations are net fiscal contributors or net fiscal claimants. The synthesis draft scheduled for September extends this battery to tax-mix reforms (variations in $\tau^{\omega}$, $\tau^{c}$, $\tau^{k}$), retirement-age shifts ($j_{R}$), joint health-pension reforms designed to evaluate policy complementarities, and a formalization experiment that expands the contributory base using observed income and health profiles from national household surveys, identifying the threshold formalization rate $\alpha^{*}$ at which the contribution-base effect dominates the expenditure-cost effect on $\tau^{p}$.
...
The general-equilibrium solve uses the endogenous grid method on the
inner household problem and a damped fixed point on $(K, L)$ for the
outer block, with damping coefficient 0.5 and convergence tolerance
$10^{-4}$ on the goods-market residual scaled by $Y$. Convergence is
attained in 13 outer iterations from the initialization
$(K_{0}, L_{0}) = (12.0, 10.0)$. Figure~\ref{fig:euler} reports the
Euler residual diagnostic at the converged baseline.

\begin{figure}[H]
\centering
\includegraphics[width=0.80\linewidth]{plots/ge_06_euler_residuals.png}
\caption{Euler residual diagnostic at the baseline equilibrium}
\label{fig:euler}
\begin{minipage}{0.80\linewidth}
\smallskip
\footnotesize
\textit{Notes:} Distribution of $|r|/u_{c}$ across interior grid
points at the converged household problem. The mean $\log_{10}$ of
the residual is $-7.24$, the 95th percentile is $-5.27$, and the
maximum is $-4.71$. The diagnostic supports welfare comparisons
across stationary equilibria at the precision required for the
counterfactual experiments in Section~\ref{sec:experiments}.
\end{minipage}
\end{figure}
