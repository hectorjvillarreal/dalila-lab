...
const κ_rep      = 0.30                 # [PDF] pension replacement rate
...
function write_csv(hist, A_dom, L_new, C, M, Λvoid, Y, K, L, W)
    ...
    out_dir = @__DIR__
    open(joinpath(out_dir, "ge_lifecycle_kappa30.csv"), "w") do io
    ...
    open(joinpath(out_dir, "ge_summary_kappa30.csv"), "w") do io
    ...
    open(joinpath(out_dir, "ge_history_kappa30.csv"), "w") do io
    ...
    println("  Wrote ge_lifecycle_kappa30.csv, ge_summary_kappa30.csv, ge_history_kappa30.csv  (under $(out_dir))")
end
...
function make_audit_plots(hist::GEHistory, C, M, K, Y, Λvoid, L)
    plot_dir = joinpath(@__DIR__, "plots_kappa30")
